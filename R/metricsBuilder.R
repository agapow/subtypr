...
asw.affinity <- list(name = "asw.affinity", label = "Average silhouette width (affinity matrix)",
...
asw.distance <- list(name = "asw.distance",
...
dunn$Metric <- function(res, ground.truth= NULL, plot = F) {
  fpc::cluster.stats(d = res$data.returned,
                     clustering = res$partition,
                     alt.clustering = ground.truth)$dunn
}
