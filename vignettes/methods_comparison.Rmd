...
Here, we show how to use `subtypr` functionalities to compare the performances of two methods: Similarity Network Fusion and Affinity Network Fusion (ANF).

## Data

We'll use `breast_cancer_data`. It's a breast cancer multi-omic dataset from The Cancer Genome Atlas (TCGA): https://cancergenome.nih.gov/. with known molecular subtypes.
We'll also use `gbm_data`, a glioblastoma multiforme multi-omic dataset from TCGA too with no molecular subtypes here but with survival data of the patients.
...
```{r grid_anf_gbm}
grid_support_anf <- list(cluster_number = 2:8, 
                         k_affi = seq(10, 25, 5), k_fusion = seq(10, 25, 5))
```
...
result_anf <- tuning(data_list = gbm_data$data_list, 
                     method = "subtype_anf", 
                     grid_support = grid_support_anf, 
                     metric = "asw_affinity", 
                     parallel = T, ncores = 7, 
...
result_snf <-  tuning(data_list = gbm_data$data_list, 
                     method = "subtype_snf", 
                     grid_support = grid_support_snf, 
                     metric = "asw_affinity", 
                     parallel = T, ncores = 7, 
