...
names(get_method())
```
Here, we'll apply Similiarity Network Fusion (SNF). To have more information one the method, use the documentation. For example: `?subtype_snf`.
...
A metric evaluate only partial aspects of the clustering results. Therefore, we provide `overview_metrics()` to evaluate the "best" set of parameters returned by `tuning()` with other metrics. 
Here, the only internal metric suitable for affinity matrix is the metric we used for tuning and we don't have a ground truth... but we have the survival data. We'll use `analyze_survival()`:

```{r analyze_survival}
analyze_survival(survival_time = survival_data$Survival, 
                 death_status = survival_data$Death, 
                 patients_partition = result_tuning$method_res$partition)
  
```

<!-- ## Comparison of methods -->

<!-- Here, we'll compare the performance of two methods: SNF and Affinity Network Fusion (ANF). The comparison of methods is often done without tuning properly the "adversary" methods. The core idea here is that we'll use `tuning()` to use each methods at their full potential. -->
